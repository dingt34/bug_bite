...
assert.strictEqual(page.data.actionOptions.includes('未处理'), true);
...
page.onActionTap({ currentTarget: { dataset: { v: '未处理' } } });
assert.deepStrictEqual(page.data.actionsTaken, ['未处理']);
page.onActionTap({ currentTarget: { dataset: { v: '冷敷' } } });
assert.deepStrictEqual(page.data.actionsTaken, ['冷敷']);
