...
function validDate(value) { return /^\d{4}-\d{2}-\d{2}$/.test(value || '') && !Number.isNaN(new Date(value).getTime()); }
...
Page({
  data: { destination: '', date: '', dateValue: '', minDate: '', activity: '', habitats: [], overnight: '当日往返', companions: [], gears: [], route: null, expanded: false, submitting: false, coverageMessage: '', saveStatus: '填写目的地、日期和活动即可生成清单', habitatOptions: [], overnightOptions: [], companionOptions: [], gearOptions: [] },
  ...
  applyPlan(plan) {
    ...
    this.setData({ destination, dateValue, date: displayDate(dateValue), minDate: today(), activity: ACTIVITIES.indexOf(plan.activityType || plan.activity) >= 0 ? (plan.activityType || plan.activity) : '', habitats, overnight, companions, gears, route: plan.route || null, expanded: Boolean(habitats.length || companions.length || gears.length || overnight !== '当日往返'), coverageMessage: destination && !matchedRegions(destination).length ? 'P0 当前仅覆盖浙江省内目的地，请填写具体城市或地点。' : '', habitatOptions: options(HABITATS, habitats), overnightOptions: options(OVERNIGHT, overnight), companionOptions: options(COMPANIONS, companions), gearOptions: options(GEARS, gears) });
  },
  ...
  chooseActivity() { wx.showActionSheet({ itemList: ACTIVITIES, success: result => this.change({ activity: ACTIVITIES[result.tapIndex] }) }); },
  ...
  generate() {
    ...
    store.set('plans', plans); store.set('currentPlan', plan); store.remove('precheckDraft'); this.planId = plan.id; this.completed = true;
