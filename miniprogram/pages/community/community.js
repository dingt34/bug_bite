...
function highlightText(value, terms) {
  ...
  if (!terms.length || !text) return [{ name: 'span', attrs: {}, children: [{ type: 'text', text }] }];
  const matcher = new RegExp('(' + terms.map(escapeRegExp).join('|') + ')', 'ig');
  return text.split(matcher).filter(Boolean).map(part => ({ name: 'span', attrs: { class: terms.some(term => part.toLowerCase() === term.toLowerCase()) ? 'search-hit' : '' }, children: [{ type: 'text', text: part }] }));
}
...
Page({
  ...
  applyFilters() {
    ...
    const posts = this.data.allPosts.filter(item => {
      const text = [item.author, item.nickname, item.region, item.type, item.stage, item.title, item.text, item.route].join(' ');
      ...
      return regionMatch && typeMatch && stageMatch && (!this.data.routeOnly || !!item.route) && (!terms.length || terms.some(term => text.toLowerCase().includes(term.toLowerCase())));
    }).map(item => Object.assign({}, item, { authorNodes: highlightText(item.author, terms), titleNodes: highlightText(item.title, terms), textNodes: highlightText(item.text, terms), routeNodes: highlightText(item.route, terms) }));
