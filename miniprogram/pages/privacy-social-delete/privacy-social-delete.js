...
Page({
  ...
  load() {
    ...
    Object.keys(allComments).forEach(postId => (allComments[postId] || []).forEach((item, index) => {
      if ((item.author || '我') === nickname || !item.author) comments.push({ id: item.id || '', postId, index, text: item.text, time: timeText(item.createdAt), postTitle: (postMap[postId] || {}).title || '已删除的经历' });
    }));
