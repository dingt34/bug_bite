...
Page({
  ...
  openSafetyCard() {
    // 如果有即将出发的行程，传递ID
    const upcomingId = this.data.upcomingDraft?.id;
    
    if (upcomingId) {
      wx.navigateTo({
        url: `/pages/offline-safety/offline-safety?id=${upcomingId}`
      });
    } else {
      wx.navigateTo({
        url: '/pages/offline-safety/offline-safety'
      });
    }
  },
