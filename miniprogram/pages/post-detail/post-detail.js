...
Page({
  ...
  chooseCommentReport(commentId) {
    wx.showActionSheet({
      ...
      success: result => {
        const reason = this.data.reportReasons[result.tapIndex];
        if (reason === '其他') {
          wx.showModal({ title: '请输入举报原因', editable: true, placeholderText: '请具体说明举报内容', success: modal => { const custom = String(modal.content || '').trim(); if (modal.confirm && custom) this.confirmCommentReport(commentId, custom); } });
        } else this.confirmCommentReport(commentId, reason);
      }
    });
  },
    ...
    if (wx.showActionSheet) {
      wx.showActionSheet({
        ...
        success: result => {
          const reason = this.data.reportReasons[result.tapIndex];
          if (reason === '其他') {
            wx.showModal({ title: '请输入举报原因', editable: true, placeholderText: '请具体说明举报内容', success: modal => { const custom = String(modal.content || '').trim(); if (modal.confirm && custom) this.confirmReport(custom); } });
          } else this.confirmReport(reason);
        }
      });
      return;
    }
