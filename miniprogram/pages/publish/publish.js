...
Page({
  ...
  selectPicker(e) { const index = Number(e.currentTarget.dataset.index); const key = this.data.pickerKey; if (key === 'event') { const event = (this.pickerEvents || [])[index]; if (event) this.setData({ selectedEventId: event.id, selectedEventLabel: eventLabel(event), selectedEventImages: this.eventImages(event), type: eventType(event) || this.data.type, stage: eventStage(event) || this.data.stage, region: event.region || this.data.region, pickerVisible: false }); return; } if (key === 'route') { const route = (this.pickerRoutes || [])[index]; if (!route) { this.closePicker(); this.waitingForRoute = true; wx.navigateTo({ url: '/pages/route/route?from=publish' }); return; } this.setData({ route, routeLabel: routeLabel(route), region: routeRegion(route) || this.data.region, pickerVisible: false }); return; } this.setData({ [key]: this.data.pickerItems[index], pickerVisible: false }); },
